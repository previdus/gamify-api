...
public class AnswerKeyDAOImpl extends
		HibernateGenericRepository<AnswerKey, Serializable> implements
		AnswerKeyDAO {

	private static final Logger log = LoggerFactory
			.getLogger(AnswerKeyDAOImpl.class);
	...
	public boolean isCorrectAnswer(Long questionId, Option answer) {
		...
		log.info("Question Id " + questionId + " Correct Answer "
				+ getAnswerKey(new Question(questionId)).getOptionId()
	...
	public AnswerKey getAnswerKey(Question question) {
		try {
			...
			Query qry = getSession().createQuery(
					"from AnswerKey where questionId = :quesId").setParameter(
		...
		/*
		 * Option option =
		 * GameStub.questionCorrectAnswerMap.get(question.getId()); AnswerKey
		 * answerKey = new AnswerKey(); answerKey.setAnswer(option);
		 * answerKey.setQuestion(question); return answerKey;
		 */
	}
}
