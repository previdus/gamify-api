...
public class GenerateDummyDataInDatabaseImpl  implements GenerateDummyDataInDatabase{
	...
	public void saveDummyGI(){
      ...
      //  gameInstanceDAO.detach(gi);
        ...
        PreviousQuestionLog pql1 = new PreviousQuestionLog(new Question(2l), null, new User(1l) ,gi );
        pql1.addPlayerResponses(new PlayerResponseLog(new User(1l), new Option(1l),1l, 1));
        pql1.addPlayerResponses(new PlayerResponseLog(new User(2l), new Option(2l),2l, 2));
       // logs.add(new PreviousQuestionLog(new Question(3l), null, new User(2l), new AnswerKey(3l, 1l) ,gi ));
        logs.add(pql1);
	...
	private void saveQuestion(Topic topic, int j){
		...
		for(int i = 0; i <4; i++){				
			options.add(optionDAO.saveNew(new Option("option"+i,null,(i+1), question)));
		}
		correctOption = optionDAO.saveNew(new Option("option"+4,null,(4+1), question));
