...
public class ApiLmsGameController {
	private static final Logger log = LoggerFactory.getLogger(ApiLmsGameController.class);
	@Autowired
	...
	@ResponseBody
	public GamePageResult selectRoom(@PathVariable("userToken") String userToken,@PathVariable("examSection") String examSection) {
			...
			if(user != null){
				if(GameQueueManager.checkIfPlayerAlreadyInGame(user)){
					...
					gp.setUser(user);
					 ...
					 ExamSection es = examSectionService.getExamSection(new Long(examSection));
					 if(es != null){
	...
	@ResponseBody
    public  GamePageResult pollGameInstance(@PathVariable("userToken") String userToken) 
    		throws  IOException 
    {
		GameInstance gi = null;
		GamePageResult gp = new GamePageResult();
		...
		if(user != null){	  
	         ...
	         if(gi != null){
	        	 if(gi.getCurrentQuestion() != null)
	        		 log.debug("----- Current Question Id is : " + gi.getCurrentQuestion().getId());
	...
	@ResponseBody
    public GamePageResult respondToQuestion(@PathVariable("userToken") String userToken, @PathVariable("questionId") String questionId,
    		@PathVariable("optionId") String optionId, @PathVariable("timeTakenToRespond") String timeTakenToRespond) 
    		throws  IOException 
    {
		...
		gp.setUserToken(userToken);
		if(user != null){
			GameInstance gi = GameQueueManager.recordPlayerResponseToQuestion(user.getId(), new Long(questionId), new Long(optionId), new Long(timeTakenToRespond));	        
	        if(gi == null){
