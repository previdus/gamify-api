...
public class QuestionManager{
	...
	private static AnswerKeyService answerKeyService;
	  /**
	   * Sets the answerKeyService This method should never be called except by Spring
	  ...
	  /**
	   * Sets the questionService This method should never be called except by Spring
	   * @param userAccessor The user accessor to set
	   */
	  @Autowired(required = true)
	...
	public static void savePreviousQuestionLog(GameInstance gi){
		...
		try{
   	    // AnswerKey answerKey = answerKeyService.getAnswerKey(gi.getCurrentQuestion());
	   	 ...
	   	 pql.setPlayersResponses(new ArrayList<PlayerResponseLog>(gi.getPlayerResponsesToCurrentQuestion().values()));
	   	 pql.setBestTime(new Long(gi.getBestTimeForCurrentQuestion()).toString());
	   	 pql.setWinner(gi.getCurrentQuestionWinner());
	   	 List<PreviousQuestionLog> logs = GameQueueManager.gameResponseLog.get(gi.getId());
	   	 if(logs == null)
	   		logs = new LinkedList<PreviousQuestionLog>();
	   	 logs.add(pql);
	   	  GameQueueManager.gameResponseLog.put(gi.getId(), logs);
	   	 log.info("Prev Question Log Size after adding " + GameQueueManager.gameResponseLog.get(gi.getId()).size());
		...
		log.info("***************ENTERED - attachQuestionToGameInstance()***************");
		 ...
		 if(questions== null || questions.size() == 0){
			 ...
			 try{
				 for(Topic topic: topics){
					 ...
					 try{
						 thisTopicQuestions = questionService.getQuestions(topic);	 
					 }
					 catch(Exception q){
						 q.printStackTrace(); 	
					 }
					 if(thisTopicQuestions != null && thisTopicQuestions.size() > 0){
		 ...
		 if(questions.size() > 0){	
			 ...
			 if(gi.getCurrentQuestion() == null){
				 ...
				 while(true){
					 newCurrentQuestion = questions.get(r.nextInt(questions.size()));
				     if(  !newCurrentQuestion.getId().equals(gi.getCurrentQuestion().getId())){
				    	 //attach new question
